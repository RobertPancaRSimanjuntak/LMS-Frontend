...
export default {
    ...
    methods: {
        ...
        saveAttempt() {
            const attempts = JSON.parse(localStorage.getItem('quiz1_attempts')) || [];
            attempts.push({
                score: this.score,
                totalQuestions: this.questions.length,
                date: new Date().toLocaleString()
            });
            localStorage.setItem('quiz1_attempts', JSON.stringify(attempts));
        },
        isCorrect(index) {
        ...
        goBack() {
            ...
            this.$router.push({ name: 'KuisCard1' });
        }
    }
};
</script>
