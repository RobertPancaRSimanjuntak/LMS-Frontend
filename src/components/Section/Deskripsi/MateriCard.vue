  ...
  <div class="flex flex-col space-y-8 justify-end mr-8">
    <div class="col-span-5 xl:col-span-2">
      <div class="flex justify-between items-center px-4">
        <div class="py-4">
          <h3 class="font-semibold text-lg text-black">Sejarah Algoritma</h3>
        </div>
      </div>
      <div class="rounded-sm border border-stroke bg-white shadow-default">
        ...
        <div class="p-7">
          <form @submit.prevent="handleUrlSubmit">
            <!-- User Url Section -->
            <div v-if="userUrl" class="mb-4 flex items-center justify-center">
              <!-- Embedded Url -->
              <video controls :src="userUrl" class="w-full max-h-screen"></video>
            </div>
            <!-- File Upload Section -->
            <div v-else>
              <div class="relative rounded border-2 border-dashed border-gray-300 bg-gray py-4 px-4 sm:py-8">
                <div class="flex flex-col items-center w-full h-20 space-y-1">
                  <img src="@/assets/pdf1.png" alt="mp4_file" class="w-8 h-8" />
                  ...
                  <div v-if="uploadedUrl" class="flex flex-col items-center justify-center">
                    <p class="text-sm font-medium">{{ uploadedUrlName }}</p>
                  </div>
                  <div v-else class="flex flex-col items-center justify-center space-y-1">
                    <p class="text-sm font-medium">MP4 only</p>
    ...
    <div class="flex justify-between mb-8">
      ...
      <router-link :to="{ name: 'KuisCard1', params: { id: id } }">
        <div class="text-center justify-end text-md hover:bg-blue-300 bg-blue-200 text-[#564FFD] rounded py-2 px-6">
          Selanjutnya
        </div>
      </router-link>
...
const userUrl = ref(require('@/assets/Sejarah.mp4'));
const uploadedUrl = ref(null);
const uploadedUrlName = ref();

const handleUrlSubmit = () => {
  if (uploadedUrl.value) {
    userUrl.value = uploadedUrl.value;
    uploadedUrl.value = null; // Clear the uploaded URL
    uploadedUrlName.value = '';
  }
};
...
const handleFileChange = (event) => {
  ...
  if (file && file.type === 'video/mp4') {
    ...
    reader.onload = (e) => {
      uploadedUrl.value = e.target.result;
    };
    reader.readAsDataURL(file);
    uploadedUrlName.value = file.name;
  } else {
...
const saveButtonClass = computed(() => {
  return uploadedUrl.value ? 'bg-blue-400 hover:bg-blue-500' : 'bg-blue-200';
});
</script>
