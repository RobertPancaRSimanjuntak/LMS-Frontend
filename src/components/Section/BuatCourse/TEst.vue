            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default">
                <div class="flex flex-col border-b px-4 border-stroke">
                    <div class="flex justify-between items-center py-4">
                        <h3 class="font-medium text-black">Test</h3>
                    ...
                    <div class="flex flex-col items-end">
                        <div
                            class="flex justify-end items-center text-center space-x-2 bg-blue-600 my-4 py-2 px-4 max-w-max cursor-pointer"
                            @click="addQuestion">
                ...
                <div v-for="(question, qIndex) in questions" :key="qIndex" class="flex flex-col gap-4 md:gap-8 mx-8 my-4 p-8 rounded-md bg-gray-100">
                    ...
                    <div class="flex flex-col gap-1">
                        <div v-for="(option, oIndex) in question.options" :key="oIndex" class="flex justify-between py-2 px-4 bg-white text-center items-center">
                            <input
                                v-model="option.text"
                            ...
                            <div class="flex justify-between items-center space-x-2 lg:space-x-4">
                                <i 
                                    class="fa-solid fa-circle-check w-8 h-auto cursor-pointer" 
                                    :class="{'text-green-500': option.correct, 'text-gray-400': !option.correct}"
                                    @click="toggleCorrectAnswer(qIndex, oIndex)">
                                </i>  
                                <i class="fa-regular fa-trash-can text-red-600 cursor-pointer" @click="removeOption(qIndex, oIndex)"></i>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="flex justify-end gap-4 mx-8 my-4">
                    <button 
                    class="flex justify-center rounded py-2 px-4 text-sm text-gray bg-blue-200" @click="saveQuiz">
                        Save
                    </button>
                </div>
            </div>
